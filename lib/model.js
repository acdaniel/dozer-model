...
function define (definition) {
  ...
  return {
    ...
    update: function (query, update, options) {
      ...
      debug('updating ' + JSON.stringify(query) + ' from ' + uri);
      return dozer.put(uri, update, options);
    },
    ...
    function addVirtuals (def, obj) {
      if (def.virtuals) {
        Object.keys(def.virtuals).forEach(function (name) {
          obj[name] = def.virtuals[name].call(obj);
        }.bind(this));
      }
      if (def.properties) {
    ...
    return this.validate(options)
      .then(function (value) {
        ...
        if (!this.isNew()) {
          ...
          var update = { $set: set };
          var uri = definition.collectionUri + '/' + this._id.toString();
...
function _extend (definition, superDef) {
  ...
  if (!newDef.collectionUri) {
    newDef.collectionUri = superDef.collectionUri;
  }
